...
import { clc, isFunction, isPlainObject, isString, isUndefined, yellow } from '@vani0-0/utils'
...
export class ConsoleLogger implements LoggerService {
  ...
  private getContextAndMessagesToPrint(args: unknown[]): IMessages {
    if (args?.length <= 1) {
  ...
  private getContextAndStackAndMessagesToPrint(args: unknown[]): IMessages {
    if (args.length === 2) {
  ...
  private getColorByLogLevel(level: LogLevel): (text: string) => string {
    switch (level) {
      case 'debug':
